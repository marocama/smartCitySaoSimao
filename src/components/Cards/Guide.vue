<template>
  <article class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-md" @mouseenter="arrows = true" @mouseleave="arrows = false">
    ...
    <div class="py-2 grid grid-cols-2 divide-x bg-gray-50 text-gray-500 antialiased">
      <a :href="`https://www.google.com/maps/search/?api=1&query=${data.Address ? `${data.Address.Street}, ${data.Address.Number} - ${data.Address.City} / ${data.Address.State}` : ``}`" target="_blank" class="flex justify-center items-center space-x-2 hover:text-purple-500">
      ...
      <a :href="`${data.Phone && data.Whatsapp ? `https://api.whatsapp.com/send?phone=55${data.Phone.replace(/-| |\(|\)/ig, '')}` : `tel:${data.Phone ? data.Phone.replace(/-| |\(|\)/ig, '') : ''}`}`" target="_blank" class="flex justify-center items-center space-x-2 hover:text-purple-500">
